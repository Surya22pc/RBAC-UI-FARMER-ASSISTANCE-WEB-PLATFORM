    ...
    useEffect(() => {
        ...
        const fetchUsersAndProducts = async () => {
            try {
                ...
                const response1 = await getProducts(token);
                setProducts(response1.products.length);
            } catch (error) {
    ...
    return (
        <div className=" w-full ">
            <div className="flex justify-between px-4">
                ...
                <div className="bg-white px-4 py-2 rounded-md shadow-md">
                <h1 className="text-[#219C90] text-lg font-medium">Total Products: <strong>{products}</strong></h1>
                </div>
            </div>
            <div className="flex flex-col justify-center gap-2 p-10 text-center md:flex-row ">
                <div className="w-1/2 ">
                    <img src={image} alt="Welcome" className="w-3/4 h-auto object-cover rounded-lg mb-6"/>
                </div>
