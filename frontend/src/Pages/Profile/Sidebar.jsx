...
const Sidebar = () => {
    ...
    return (
        <div className='overflow-y-auto h-auto' style={{ scrollbarWidth: "none", msOverflowStyle: "none" }}>
            <button
                className="fixed left-2 z-50 bg-[#8EC44C] text-white p-1 rounded-full md:hidden mt-1"
                onClick={toggleSidebar}
            >
                {isOpen ? <IoMdClose className="text-xl" /> : <IoIosArrowForward className="text-xl" />}
            </button>
            <div className={`navbar-container fixed ${isOpen ? "top-20 md:top-20 " : "top-20 md:top-1"} left-0 h-full z-40 bg-[#F9FAFB] shadow-md transform ${isOpen ? "translate-x-0 " : "-translate-x-full"} transition-transform duration-300 ease-in-out md:relative md:translate-x-0 w-40 md:w-40 overflow-hidden `} style={{ scrollbarWidth: "none", msOverflowStyle: "none" }}>
